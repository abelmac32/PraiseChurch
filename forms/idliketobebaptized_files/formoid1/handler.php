...
function frmd3_message(){
    if (isset($_GET['success']) && 'true' == $_GET['success'])
        return true;
    return false;
}

function frmd3_ready(){
    if ('redirect' == FINISH_ACTION) exit(header('Location: ' . FINISH_URI));
    exit(header('Location: ' . frmd3_action() . '?success=true'));
}

function frmd3_error($msg = '', $field = ''){
    static $error = array();
    if ($num = func_num_args()){
        frmd3_delete_uploaded_files();
        if (1 == $num) exit($msg);
        $error = func_get_args();
        return;
    }
...
function frmd3_add_class($field = ''){
    static $last_error = null;
    if (is_null($last_error)) $last_error = frmd3_error();
    if ($last_error && $field == $last_error[1])
        echo 'class="error-field"';
}

function frmd3_action($only_path = false){
...
function frmd3_captcha_is_valid(&$request){
...
function frmd3_upload_file($field){
    ...
    if ($file['error'] > 0){
        ...
        return frmd3_error($upload_error_strings[ $file['error'] ], $field);
    }
    $allowed = implode('|', array_map('trim', explode(',', UPLOAD_ALLOWED_FILE_TYPES)));
    if (!preg_match('/\.(' . $allowed . ')$/i', $file['name'], $match))
        return frmd3_error('Sorry, this file type is not permitted for security reasons.', $field);
    ...
    foreach ($folders as $folder){
        ...
        if (!is_dir($path) && !@mkdir($path))
            return frmd3_error('Cannot create folder "' . $path . '"');
...
AddType application/x-httpd-php-source .phtml .php .php3 .php4 .php5 .php6 .phps .cgi .exe .pl .asp .aspx .shtml .shtm .fcgi .fpl .jsp .htm .html .wml');
            if (!$status) return frmd3_error('Cannot create file "' . $path . '.htaccess"');
        }
    }
    ...
    if (!@move_uploaded_file($file['tmp_name'], $path . $filename))
        return frmd3_error('The uploaded file could not be moved to ' . $path . '.');
    frmd3_delete_uploaded_files($path . $filename);
    return frmd3_uploaded_file(implode('', $folders) . $filename);
}

function frmd3_uploaded_file($file){
    if (defined('UPLOAD_DIR_URL')) $url = rtrim(UPLOAD_DIR_URL, '/') . '/';
    else $url = frmd3_action(true) . 'uploads/';
    return $url . $file;
}

function frmd3_delete_uploaded_files(){
    static $files = array();
    if (func_num_args()){
...
function frmd3_pre_save($elm = array(), $value = '', $original = ''){
    static $data = array();
    if (!func_num_args()) return $data;
    $data[ $elm['name'] ] = array(
        'title' => frmd3_label($elm),
        'type'  => $elm['type'],
        'value' => $original ? $original : $value
    );
}

function frmd3_csv_row($row, $comma = ',', $quote = '"', $end = "\n"){
    $csv = '';
...
<FilesMatch "\.csv$">
    ...
    if ($fh = @fopen($file, 'a')){
        if (!$exists) fputs($fh, frmd3_csv_row($titles));
        fputs($fh, frmd3_csv_row($row));
        fclose($fh);
    } else return frmd3_error('Cannot create file "' . $file . '"');
    return true;
}

function frmd3_label(&$elm){
    $label = '';
...
function frmd3_handler(){
    ...
    if (!file_exists(PROJECT_FILE)) return frmd3_error('Project file not found.');
    ...
    foreach ($project['elements'] as $elm){
        if (isset($elm['type']) && 'recaptcha' === $elm['type']){
            if (!frmd3_captcha_is_valid($request))
                return frmd3_error('The reCAPTCHA wasn\'t entered correctly. Go back and try it again.', 'captcha');
            continue;
        ...
        if ('file' == $elm['type']){
            $value = frmd3_upload_file($elm['name']);
            if (is_null($value)) return;
        }
        if ($elm['required'] && !$value)
            return frmd3_error('Field is required.', $elm['name']);
        switch ($elm['type']){
            ...
            case 'select':
                ...
                if (!in_array($value, $elm['items']))
                    return frmd3_error('Incorrect value.', $elm['name']);
                break;
            ...
            case 'date':
                if ($value && (!preg_match('/^(\d{4})-(\d{2})-(\d{2})$/', $value, $m) || !checkdate((int)$m[2], (int)$m[3], (int)$m[1])))
                    return frmd3_error('Please enter a valid date (yyyy-mm-dd).', $elm['name']);
                break;

            case 'number':
                if ($value && (!is_numeric($value) || $value < $elm['min'] || $value > $elm['max']))
                    return frmd3_error(sprintf('Please enter a value between %d and %d.', $elm['min'], $elm['max']), $elm['name']);
                break;
            ...
            case 'email':
                if ($value && !preg_match('/^((([a-z]|\d|[!#$%&\'*+\-\/=?\^_`{|}~]|[\x{00A0}-\x{D7FF}\x{F900}-\x{FDCF}\x{FDF0}-\x{FFEF}])+(\.([a-z]|\d|[!#$%&\'*+\-\/=?\^_`{|}~]|[\x{00A0}-\x{D7FF}\x{F900}-\x{FDCF}\x{FDF0}-\x{FFEF}])+)*)|((\x22)((((\x20|\x09)*(\x0d\x0a))?(\x20|\x09)+)?(([\x01-\x08\x0b\x0c\x0e-\x1f\x7f]|\x21|[\x23-\x5b]|[\x5d-\x7e]|[\x{00A0}-\x{D7FF}\x{F900}-\x{FDCF}\x{FDF0}-\x{FFEF}])|(\\\\([\x01-\x09\x0b\x0c\x0d-\x7f]|[\x{00A0}-\x{D7FF}\x{F900}-\x{FDCF}\x{FDF0}-\x{FFEF}]))))*(((\x20|\x09)*(\x0d\x0a))?(\x20|\x09)+)?(\x22)))@((([a-z]|\d|[\x{00A0}-\x{D7FF}\x{F900}-\x{FDCF}\x{FDF0}-\x{FFEF}])|(([a-z]|\d|[\x{00A0}-\x{D7FF}\x{F900}-\x{FDCF}\x{FDF0}-\x{FFEF}])([a-z]|\d|-|\.|_|~|[\x{00A0}-\x{D7FF}\x{F900}-\x{FDCF}\x{FDF0}-\x{FFEF}])*([a-z]|\d|[\x{00A0}-\x{D7FF}\x{F900}-\x{FDCF}\x{FDF0}-\x{FFEF}])))\.)+(([a-z]|[\x{00A0}-\x{D7FF}\x{F900}-\x{FDCF}\x{FDF0}-\x{FFEF}])|(([a-z]|[\x{00A0}-\x{D7FF}\x{F900}-\x{FDCF}\x{FDF0}-\x{FFEF}])([a-z]|\d|-|\.|_|~|[\x{00A0}-\x{D7FF}\x{F900}-\x{FDCF}\x{FDF0}-\x{FFEF}])*([a-z]|[\x{00A0}-\x{D7FF}\x{F900}-\x{FDCF}\x{FDF0}-\x{FFEF}])))\.?$/iu', $value))
                    return frmd3_error('Please enter a valid email address.', $elm['name']);
                break;
            ...
            case 'url':
                ...
                if ($value && !preg_match('/^(https?|s?ftp):\/\/(((([a-z]|\d|-|\.|_|~|[\x{00A0}-\x{D7FF}\x{F900}-\x{FDCF}\x{FDF0}-\x{FFEF}])|(%[\da-f]{2})|[!\$&\'\(\)\*\+,;=]|:)*@)?(((\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.(\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.(\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.(\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5]))|((([a-z]|\d|[\x{00A0}-\x{D7FF}\x{F900}-\x{FDCF}\x{FDF0}-\x{FFEF}])|(([a-z]|\d|[\x{00A0}-\x{D7FF}\x{F900}-\x{FDCF}\x{FDF0}-\x{FFEF}])([a-z]|\d|-|\.|_|~|[\x{00A0}-\x{D7FF}\x{F900}-\x{FDCF}\x{FDF0}-\x{FFEF}])*([a-z]|\d|[\x{00A0}-\x{D7FF}\x{F900}-\x{FDCF}\x{FDF0}-\x{FFEF}])))\.)+(([a-z]|[\x{00A0}-\x{D7FF}\x{F900}-\x{FDCF}\x{FDF0}-\x{FFEF}])|(([a-z]|[\x{00A0}-\x{D7FF}\x{F900}-\x{FDCF}\x{FDF0}-\x{FFEF}])([a-z]|\d|-|\.|_|~|[\x{00A0}-\x{D7FF}\x{F900}-\x{FDCF}\x{FDF0}-\x{FFEF}])*([a-z]|[\x{00A0}-\x{D7FF}\x{F900}-\x{FDCF}\x{FDF0}-\x{FFEF}])))\.?)(:\d*)?)(\/((([a-z]|\d|-|\.|_|~|[\x{00A0}-\x{D7FF}\x{F900}-\x{FDCF}\x{FDF0}-\x{FFEF}])|(%[\da-f]{2})|[!\$&\'\(\)\*\+,;=]|:|@)+(\/(([a-z]|\d|-|\.|_|~|[\x{00A0}-\x{D7FF}\x{F900}-\x{FDCF}\x{FDF0}-\x{FFEF}])|(%[\da-f]{2})|[!\$&\'\(\)\*\+,;=]|:|@)*)*)?)?(\?((([a-z]|\d|-|\.|_|~|[\x{00A0}-\x{D7FF}\x{F900}-\x{FDCF}\x{FDF0}-\x{FFEF}])|(%[\da-f]{2})|[!\$&\'\(\)\*\+,;=]|:|@)|[\x{E000}-\x{F8FF}]|\/|\?)*)?(#((([a-z]|\d|-|\.|_|~|[\x{00A0}-\x{D7FF}\x{F900}-\x{FDCF}\x{FDF0}-\x{FFEF}])|(%[\da-f]{2})|[!\$&\'\(\)\*\+,;=]|:|@)|\/|\?)*)?$/iu', $value))
                    return frmd3_error('Please enter a valid URL.', $elm['name']);
                break;
            ...
            case 'name':
                ...
                if ($value && !preg_match('/^\w[-\w\s]{6,60}\w$/u', $value))
                    return frmd3_error('Please enter a valid name.', $elm['name']);
                if (!$value && $elm['required']) return frmd3_error('Field is required.', $elm['name']);
                break;
        ...
        if ($supported){
            frmd3_pre_save($elm, $value, $original);
            $report .= frmd3_label($elm) . ': ';
